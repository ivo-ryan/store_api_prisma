...
import { CategoryRequestSchema, UpdateCategoryRequestSchema } from "./schema/CategoryRequestschema";
...
export class CategoryController {
    ...
    show: Handler = async ( req, res , next ) => {
        try {
            ...
            const category = await this.categoryService.findByIdCategory(id);
            res.json(category);
        } catch (error) {
            next(error)
        }
